...
This new hard problem, named {\it Air Cargo Load Planning with Routing, Pickup, and Delivery Problem (ACLP+RPDP)}, is mathematically modelled using standardized pallets in fixed positions, obeying the center of gravity constraints, delivering each item to its destination, and minimizing fuel consumption costs.
...
\cite{MesquitaSanches2023} explored these problems by solving them in an integrated form.
...
As already mentioned, \cite{LurkinSchyns2015} was the first work that simultaneously modelled WBP and SPDP in air cargo transport. The authors demonstrated that this problem is NP-hard and performed some experiments with real data, noting that their model offers better results than those obtained manually.
...
\cite{MesquitaSanches2023} contributed with a complex model and elaborate heuristics for the ACLP-RPDP that simultaneously solve 4 intractable sub-problems: APP, WBP, SPDP and TSP. They also compare the performances of four well known heuristics: Ant Colony Optimization, the Noising Method Optimization, the Greedy Randomized Adaptive Search Procedure, and Tabu Search.

As can be seen, except for \cite{MesquitaSanches2023}, the remaining works do not address air cargo palletization and load balancing with route optimization in a multi-leg flight plan, and none of them employ parallelization into their algorithms. This is the objective of our work: to reshape \cite{MesquitaSanches2023} solution process and algorithms to accommodate parallel features which can improve solution quality and performance.
...
\begin{algorithm}[H] 
	...
	\begin{algorithmic}[1]
		...
		\While{ $stagnant < 3$ } \label{aco:while}
			...
			\While{$runtime < 0.7s$ {\bf and} $counter > 0$} \label{aco:while2}
...
Many ACO applications update pheromone levels and the general attractiveness only when there is a solution improvement. Differently, this work updates in both cases, exploring also the negative learning approach proposed by \cite{math9040361}.
...
In Algorithm \ref{alg:greedy}, line \ref{greedy:eta} defines an array with the same size as pallets array to save the set of last edges inserted in the greedy solution.
...
\begin{algorithm}[H] 
	...
	\begin{algorithmic}[1]
		...
		\While{$|E| > 0$}
			...
			\EndIf	
		\EndWhile
		\State $AntsQueue \gets AntsQueue \cup G^a(V_k, E^{Na}_k \cup E^Q_k)$
	\end{algorithmic}
\end{algorithm}
...
In this heuristic, described in Algorithm \ref{alg:shims}, pallets $p_i$\/ are considered in non-descending order of $|p_i.d|$ (line \ref{shims:pallets}).

Figure \ref{fig:whip} represents the $n_k$\/ possible edges $e_{ij}^k$\/ of $p_i$\/ sorted by $\theta_{ij}^k$. \emph{mpShims}\/ starts with a greedy solution, stopping at the edge with index $\eta_1$ close to the local optimum (first phase). Then, considering even the edge with the index $\eta_2[i]$, it elaborates different possible complements for this pallet (second phase) and selects the best of these complements (third phase).


\begin{algorithm}[H]
	\caption{ $SolveNode(mpShims, k, G, numProcs)$}  \label{alg:shims}
	\begin{algorithmic}[1]
		
		\State $G^* \gets G(V_k, E^Q_k)$
		...
		\State $limits \gets getLimits(0.75,\ 0.98,\ numProcs)$, a list of unique limits, 1 for each process, between 0.75 and 0.98
		
		\State $shimsQueue \gets \{\}$, a queue of shims solutions to be used by the child processes
	
		\For{$lim \in limits$}
			\State $shimsSolve(G(V_k, E^Q_k),\ M,\ shimsQueue,\ lim,\ E)$
		\EndFor
		
		\State $counter \gets |limits|$
		
		\While{$runtime < 0.7s$ {\bf and} $counter > 0$}
...
In this phase, the heuristic that provided the best results, both in terms of time and quality, is based on {\it Next-Fit}, which is an approximation algorithm for the {\it Bin Packing Problem}\/ \cite{JohnsonGarey1985}. Basically, shims are created by accumulating the following edges, taking $slack[i]$\/ as a limit.
...
Line \ref{third_phase} corresponds to Shims' third phase. In the last line, \ref{enqueue}, {\it mpShims} puts the solution in the queue to be retrieved by the main process.
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\For{$x \gets \eta_1$ to $\eta_2[i]$}
				

			\State $NewShims \gets$ {\bf True}
		...
		\State $sh_w \gets shims$, where $shims \in Set$ and $\sum_{e_{ab}^k \in shims} t_b^k.w$ is maximum 		
		\State $sh_v \gets shims$, where $shims \in Set$ and $\sum_{e_{ab}^k \in shims} t_b^k.v$ is maximum
		\State $sh_{best} \gets shims$, where $shims \in \{sh_w, sh_v\}$ and $\sum_{e_{ab}^k \in x} t_b^k.s$ is maximum
...
We test our methods with 126 different instances because we have six operational scenarios with seven randomly generated item sets for three volume surpluses (1.2, 1.5, and 2.0 times aircraft volume capacities).
...
Observing Figure \ref{fig:scenario1data20}, it may be noticed that the best number of processes for {\it Scenario 1} (the closest to the {\it Utopia}) is \textbf{6}. For {\it Scenario 2}, the best number is \textbf{4}. Table \ref{tab:numProcs} presents all methods, volumes percentages and scenarios numbers of best parallel processes to be run to solve all tours.
...
\toprule
                                &{\bf Volume}         &                  &\multicolumn{6}{c}{\bf Scenarios}              &{\bf Normalized}\\
\cmidrule{4-9}		
{$method$}                      &{\bf surplus}        &{\bf Results}     &{\bf 1}&{\bf 2}&{\bf 3}&{\bf 4}&{\bf 5}&{\bf 6}&{\bf Speed-up} \\
\toprule
\multirow{6}{*}{\it {mpShims}} &\multirow{2}{*}{$1.2$}& $f$              & 5.70  & 3.09  & 2.94  & 2.90  & 2.79  & 5.70  &   \\
                               &                      &{\bf run-time (s)}& 1     & 3     & 7     & 26    & 97    & 267   &   \\
\cmidrule{2-10}                               
                               &\multirow{2}{*}{$1.5$}& $f$              & 8.15  & 4.22  & 4.40  & 4.01  & 3.51  & 7.08  &\\
                               &                      &{\bf run-time (s)}& 1     & 5     & 12    & 45    & 244   & 737   &\\
\cmidrule{2-10}                                 
                               &\multirow{2}{*}{$2.0$}& $f$              & 11.31 & 6.29  & 5.54  & 5.83  & 5.05  & 10.63 &\\
                               &                      &{\bf run-time (s)}& 2     & 14    & 34    & 106   & 477   & 2417  &\\
\midrule

\multirow{6}{*}{\it {mpACO}}   &\multirow{2}{*}{$1.2$}& $f$              &  &     &      &      &      &      &   \\
                               &                      &{\bf run-time (s)}&  &     &      &      &      &      &   \\
\cmidrule{2-10}                               
                               &\multirow{2}{*}{$1.5$}& $f$              &&&&&&&\\
                               &                      &{\bf run-time (s)}&&&&&&&\\
