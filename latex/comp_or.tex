...
{\color{blue} $k$ is the node index and $\pi_k$ is the node in position $k$ of tour $\pi$. $\tau^{\pi_k}$ is the total torque applied by the loaded pallets to the aircraft in this node. }
...
\begin{equation} \label{eq:costs}
	\tilde{c} = c_{0,\pi(1)}\times(1+c_g\times|\epsilon_0|) + \sum_{k=1}^{K-1} [ c^{\pi(k), \pi(k+1)}\times(1+c_g\times|\epsilon^{\pi_k}|) ] + c^{\pi(K),0}\times(1+c_g\times|\epsilon_K|)
\end{equation}
...
\tau^{\pi_k} = \sum_{i=1}^{m}[ p_i.d \times (\sum_{j=1}^{n^{\pi_k}} X_{ij}^{\pi_k} \times t_j^{\pi_k}.w +  \sum_{q=1}^{m} Y_{iq}^{\pi_k} \times a_q^{\pi_k}.w)];\ k \in \{0, 1, \ldots, K\}
\end{equation}
...
\begin{equation} \label{eq:pdp11}
	L_0 = L; L^{\pi_k} = L^{\pi(k-1)} - \{l^{\pi_k}\}; \ k \in \{1, 2, \ldots, K\}
\end{equation}
...
\begin{equation} \label{eq:LatIt}
	LatIt^{\pi_k} = \sum_{i=1}^{m} \sum_{j=1}^{n^{\pi_k}} ( X_{ij}^{\pi_k} \times t_j^{\pi_k}.w \times (i\%2) - X_{ij}^{\pi_k} \times t_j^{\pi_k}.w \times (i+1)\%2 ); \ k \in \{0, 1, \ldots, K\}
\end{equation}

\begin{equation} \label{eq:LatCons}
	LatCons^{\pi_k} =  \sum_{i=1}^{m} \sum_{q=1}^{m}  ( Y_{iq}^{\pi_k} \times a_q^{\pi_k}.w \times (i\%2) - Y_{iq}^{\pi_k} \times a_q^{\pi_k}.w \times (i+1)\%2); \ k \in \{0, 1, \ldots, K\}
\end{equation}

\begin{equation} \label{eq:torqlat}
	s.t.: d^{CG}_{pallet} \times | LatIt^{\pi_k} + LatCons^{\pi_k} | \leq  \sum_{i=1}^{m}p_i.w \times limit^{CG}_{lat}; \ k \in \{0, 1, \ldots, K\}
\end{equation}

\begin{equation} \label{eq:torqlong}
	s.t.: |\tau^{\pi_k}| \leq maxW \times limit^{CG}_{long};\ k \in \{0, 1, \ldots, K\}
\end{equation}

\begin{equation} \label{eq:payload}
	s.t.: \sum_{i=1}^{m} (\sum_{j=1}^{n^{\pi_k}} X_{ij}^{\pi_k} \times t_j^{\pi_k}.w + \sum_{q=1}^{m_k} Y_{iq}^{\pi_k} \times a_q^k.w ) \leq maxW; \ k \in \{0, 1, \ldots, K\}
\end{equation}
...
The objective of this problem is to find a permutation $\pi \in S_K$\/ that maximizes the function $f_\pi(\tilde{s},\tilde{c})$ \ref{eq:maxf}. In this way, the flight plan will be $l_0, l_{\pi(1)}, \ldots, l^{\pi_k}, l_0$. $\tilde{s}$\/ is the total score of transported items \ref{eq:scores} and  $\tilde{c}$\/ is the total cost of fuel consumed \ref{eq:costs}. As can be seen, $\tilde{c}$\/ corresponds to the fuel consumption due to the flights carried out and the CG deviation of the transported cargo. Throughout this work, for simplicity, we use $f=\tilde{s}/\tilde{c}$.
