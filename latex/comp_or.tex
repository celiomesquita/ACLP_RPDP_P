...
\begin{table}[H]
	...
	\begin{tabular}{MT}
		...
		\midrule
\multicolumn{2}{l}{--- from \cite{MesquitaSanches2023} ---}\\		
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\For {$k\gets0$ to $K$} \label{tour:loop1}		
			\For{$i \gets 1$ to $m$}
				\State $p_i.to^{\pi_k} \gets -1$ \label{tour:-1} \Comment{reset this pallet destination}
			\EndFor	
			\If {$k = 0$}
				\State $L^{\pi_0} \gets L$
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\Procedure{$mpShims$}{${N^{\pi_k}, \pi_k}, limit, G$}
		...
		\State $ts \gets 1.7$ \Comment{torque surplus} \label{mpShims:ts}
		...
		\State $minCGdev(M^{\pi_k},\ \pi_k,\ nodeTorque)$  \label{mpShims:minCGdev} \Comment{minimize the CG deviation.}
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\Procedure{$getBestShims$}{$i,\ \eta_1,\ \eta_2,\ E,\ \pi_k,\ slack$}
		...
		\If { $e_{ij}^{\pi_k} \not\in (E_N^{\pi_k} \cup shims)$ {\bf and} $e_{ij}^{\pi_k}$ is feasible  {\bf and}  $(t_j^{\pi_k}.v + volume) \leq slack^i$}
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\State $proc_i.fork$ \Comment{pallet $p_i$ packing processes is forked}
		\EndFor
...
\begin{table}[H]
	...
	\begin{tabular}{crcccccccc}
		...
		&                       $MIP+3Dp$     & {\it run time (s)} &     12     &     51    &     81    &     258   &      601  &     3161  &{\bf 1.15}\\% 	4164				
		\cmidrule{2-10}		                       
		&\multirow{2}{*}{ $Shims+3Dp$}        & $f$                &    9.05   &   5.11    &   9.75    &   10.96   &   12.86   &   13.25   &     0.86 \\% 
		&                                 & {\it run time (s)} &     16    &     68    &     94    &    323    &    719    &    3549   &     1.00 \\% 	4769		
		\cmidrule{2-10}		                       
		&\multirow{2}{*}{ $mpShims+3Dp$}      & $f$                &{\bf 12.14}&{\bf 7.29} &   10.36   &{\bf 12.63}&{\bf 14.04}&{\bf 13.92}&{\bf 0.99}\\% best 7080
		...
		&                      $MIP+3Dp$      & {\it run time (s)} &    20     &     83    &     142   &     362   &     996   &     4741  &{\bf 1.22} \\% 6344					
		\cmidrule{2-10}		                       
		&\multirow{2}{*}{ $Shims+3Dp$}        & $f$                &   11.55   &   7.61    &   12.28   &   15.00   &   17.55   &   17.57   &     0.84  \\%  
		&                                 & {\it run time (s)} &    25     &    107    &     156   &    498    &    1217   &   5743    &     1.00  \\% 7746
		\cmidrule{2-10}		                       
		&\multirow{2}{*}{ $mpShims+3Dp$}      & $f$                &{\bf 17.05}&   10.01   &   15.01   &   16.75   &{\bf 20.09}&{\bf 17.85}&{\bf 1.00} \\%   best 9708
		&                                 & {\it run time (s)} &    18     &    64     &     101   &    351    &   828     &  3879     &     1.50  \\% 5421
		\toprule
		\multirow{7}{*}{$2.0$}&$Relaxed$  & $f$                &     23.73 &{\bf 15.23}&   21.96   &   24.22   &   26.69   &    24.29  &     0.99  \\%  
		&                       $MIP+3Dp$     & {\it run time (s)} &    28     &     99    &     185   &    648    &   1680    &   8774    &     1.00  \\% 11414					
		\cmidrule{2-10}		                       
		&\multirow{2}{*}{ $Shims+3Dp$}        & $f$                &   16.28   &   10.93   &   18.16   &   22.35   &   25.12   &   24.53   &     0.85  \\%  
		&                                 & {\it run time (s)} &    37     &    141    &   196     &     653   &   1453    &   6851    &     1.22  \\% 9331
		\cmidrule{2-10}		                       
		&\multirow{2}{*}{ $mpShims+3Dp$}      & $f$                &{\bf 24.52}&   15.18   &{\bf 21.99}&{\bf 24.61}&{\bf 26.94}&{\bf 24.83}&{\bf 1.00} \\% best 13812
		&                                 & {\it run time (s)} &    27     &    87     &   146     &    496    &   1080    &   5160    &     1.63  \\% 6996
		\bottomrule	
	\end{tabular}
	\normalsize
\end{table}
