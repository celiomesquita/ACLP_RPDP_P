...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\Procedure{$ACLP+RPDP$}{$scenario,\ surplus,\ numProcs$}
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\Procedure{$SolveTour$}{$\pi, L, M, C, N, method, numProcs$}
...
\begin{algorithm}[H] 
	...
	\begin{algorithmic}[1]
		...
		\Procedure{$SolveNode$}{$mpACO, k, G, numProcs$}
		...
		\While{ $stagnant < 3$ } \label{aco:while}
			...
			\While{$counter > 0$} \label{aco:while2}
				...
				\While{$runtime < 0.7s$} \Comment{Wait until there is a solution in the queue}
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\Procedure{$SolveNode$}{$mpShims, k, G, numProcs$}
