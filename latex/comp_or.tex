...
We converted {\it Shims} into a process-based parallel computing heuristic that quickly finds good solutions for a wide range of problem sizes. In addition, we applied a 3D packing procedure to guarantee that only the items that fit on each pallet are present in the solution.
...
(4) A post-optimization procedure for 3D packing running in parallel mode would keep the running time adequate for operational use.
...
s.t.    & Along a tour, the set of unvisited nodes is updated. \\
		...
		& {\bf The pallets destined for the next node of the tour should be put as near as possible to the cargo ramp door (this work)}.\\
		& {\bf The items allocated to each pallet should be submitted to a 3D packing procedure (this work)}.\\				
		\midrule
	\end{tabular}
	\normalsize
\end{table}
...
{\color{purple}
In line \ref{tour:3dpacking}, we reshape the solution by selecting the items that actually fit into the pallet, as the previous result ($G_3$) is an upper bound. i.e., although the sum of volumes is less than the volumetric capacity of the pallet, it is not guaranteed that all items will fit on the pallet.
}
...
Let $p_i.H$ be the maximum height for each pallet to be built, which depends on the size of the aircraft and the pallet position along the cargo bay; each {\it 463l} pallet has a width $p_i.W = 2.15m$ and a length of $p_i.L = 2.65m$.

\begin{equation} \label{eq:3dof}
	\mbox{minimize}\ \Bigg [ p_i.W \times p_i.H \times p_i.L - \sum_{j \in p_i} t_j.w \times t_j.h \times t_j.l \ \Bigg ]
\end{equation}

\begin{equation} \label{eq:3dh}
	s.t.: \sum_{j \in p_i} t_j.h \leq p_i.H 
\end{equation}

\begin{equation} \label{eq:3dw}
	s.t.: \sum_{j \in p_i} t_j.w \leq p_i.W 
\end{equation}

\begin{equation} \label{eq:3dl}
	s.t.: \sum_{j \in p_i} t_j.l \leq p_i.L 
\end{equation}

Our implementation was also based on \cite{RuizRibeiro} (who implemented the algorithm described in \cite{DubeKanavathy2006}), taking advantage of the already implemented Python classes and auxiliary procedures, but making the necessary adjustments for parallel processing.
...
Bin packing being an NP-Hard problem suggests that an exhaustive search for the optimal solution is in general computationally intractable and that there is thus no known computationally feasible optimal solution method for the problem. So other means to obtain a solution must be found. Normally, heuristic solution methods are used.

The two primary bin packing approximate algorithms that we used were the {\it First Fit Decreasing - FFD} and the {\it Best Fit - BF}. They were chosen over other approximate algorithms because they have a faster running time and produce solutions that are much closer to the optimal solution than most other heuristic algorithms.

The running time for BF and FFD are $O(n\ Log\ n)$, excluding the running time for sorting.

The $3Dpacker$ function specification referred in Algorithm \ref{alg:3dpacking} is well described in \citet{DubeKanavathy2006}. This method returns the list of packed items, as well the list of unfit items to be excluded from solution.
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\State $E_{\pi(k)} \gets E_{\pi(k)} \setminus proc_i.unfit$  \label{3dpacking:unfit} \Comment{exclude the unfit items from solution}
		\EndFor
...
In generating the items in each node, we consider 3 values for the parameter ${\it surplus}$. The results obtained for the function $f$, with the corresponding running time in seconds, are shown in Tables \ref{tab:results} (${\it surplus} = 1.2$, ${\it surplus} = 1.5$, and ${\it surplus} = 2.0$).
...
The average tour values were presented for $f$ and, for the running time, the worst result obtained. To facilitate the comparison between the methods, we added a last column in these tables, where two values are indicated:
\begin{itemize}
...
Other important contributions are that we checked four hypotheses: Computer parallelism improved {\it Shims} performance; the intermediate integer programming procedure for CG deviation minimization added to {\it Shims} algorithm enhanced it's results; but the integer programming procedure to minimize pallets' travel in the cargo bay did not improve the results; and the post-optimization procedure for 3-D packing was turned possible for operational use due to the approximation algorithms chosen and a process-based parallel computing, having each pallet as a process.
