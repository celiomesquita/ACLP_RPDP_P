...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\State Let $G(E^{Q_{\pi(k)}})$ \Comment{the solution comes with some consolidated on board}
		...
		\State $lock \gets$ multi-processing lock \Comment{for shared data among processes to avoid race condition}
