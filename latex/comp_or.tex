		...
		\begin{tikzpicture}[scale=0.8, samples=100]
			\begin{axis}[
				...
				\addplot +[
					...
					nodes near coords = {$\pgfmathprintnumber[frac]\np$},
					visualization depends on={\thisrow{np} \as \np},
					font=\small,
				]
				table[x=time, y=score, col sep=comma] {mpShims1.csv}; 
...
%
		\begin{tikzpicture}[scale=0.8, samples=100]
			\begin{axis}[
				...
				table[x=time, y=score, col sep=comma] {mpShims2.csv}; 
				
				\legend{\it parallel processes}
			\end{axis}
		\end{tikzpicture}	
	
		\caption{{\it mpShims} performance on {\it Scenarios 1 and 2}.}
	\end{center}
 
\end{figure}\label{fig:mpShims}

Observing Figure \ref{fig:mpShims}, it may be noticed that the best number of processes for {\it Scenario 1} (closest to the upper left corner) is 8. For {\it Scenario 2}, the best number is 16.
