...
\begin{table}[H]
	...
	\begin{tabular}{c c c c}
		\toprule
		{\bf Scenario} & {$K$} & {$L$} & {Shortest tours} \\		
		\midrule
		1 & 2    & \{$1,2$\}           & 0 1 2 0 \\
		2 & 3    & \{$1,2,3$\}         & 0 1 2 3 0 \\
		3 & 4    & \{$1,2,3,4$\}       & 0 4 1 2 3 0 \\
		4 & 5    & \{$1,2,3,4,5$\}     & 0 4 1 2 5 3 0 \\
		5 & 6    & \{$1,2,3,4,5,6$\}   & 0 4 1 2 6 5 3 0 \\
		\bottomrule
	\end{tabular}
\end{table}
		...
		\For{$i \in \{1,\ldots,m\}$}
			...
			\For{$q \in \{1,\ldots,m_{\pi_k}\}$}
				...
				\If{$(i, q) \in E_{Q_{\pi_k}}$} 
					...
					\State {\color{blue} $\tau^{\pi_k} \gets \tau^{\pi_k} + w_q \times D_i$ }
...
As the MIP solver was not capable of solving all scenarios in polynomial time or reached the computer's RAM (16 GiB) too early, we decided to set the Gurobi {\it MIPgap} parameter to 1\% to control the minimal quality of the returned solutions and abbreviate the solution time.

More details on the Gurobi {\it MIPgap} refer to \cite{mipgap}.

}

\begin{table}[H]
\centering
\caption{Solutions with $surplus =1.2$\/ with the direct and reverse least cost tours.}  \label{tab:20}
\footnotesize
\begin{tabular}{c|c|ccccc|c}
\toprule
$method$          & {\bf Scenarios} & {\bf 1} & {\bf 2} & {\bf 3} & {\bf 4} & {\bf 5} & \specialcell{{\bf Normalized}\\{\bf Speed-up}}   \\
\toprule
\multirow{2}{*}{\it Gurobi* } & $f$ & 8.52    & 11.70   & 13.07   & 13.54 & 12.44     &  1.00\\ % 5927
...
\bottomrule
\multicolumn{8}{c}{Average $f$ and run time values of K! tours.}\\
\toprule

\multirow{2}{*}{\it Gurobi* } & $f$ & 8.51    & 12.25   & 13.32   & 14.61  & x        & 1.00 \\ % 4869
...
\bottomrule
\multicolumn{8}{c}{*{\it MIPgap} parameter set to 1\%. The $f$ values are \textpm 1\% distant from the optimal solution}\\
...
97\% of 16GiB was the Read-Only Memory (RAM) state before we aborted the Gurobi execution on solving scenario 5 in Table \ref{tab:20}.

\begin{figure}[H]
	\centering
	\tmpframe{\includegraphics[scale=0.6]{Images/memory.png}}
	\captionof{figure}{RAM usage on solving scenario 5 by Gurobi (Table \ref{tab:20}).}
	\label{fig:memory}
\end{figure}

\begin{table}[H]
	\centering
	\caption{Solutions with $surplus=1.5$\/ with the direct and reverse least cost tours.}  \label{tab:50}
	\footnotesize
	\begin{tabular}{c|c|ccccc|c}
		\toprule
		$method$          & {\bf Scenarios} & {\bf 1} & {\bf 2} & {\bf 3} & {\bf 4} & {\bf 5} & \specialcell{{\bf Normalized}\\{\bf Speed-up}}   \\
		\toprule
		\multirow{2}{*}{\it Gurobi* } & $f$ & 11.80 & 16.74 & 18.04 & 18.86 & 16.92 & 1.00 \\ % 8236
		...
		\multirow{2}{*}{\it Gurobi* } & $f$ & 11.81 & 17.01 & 18.25 & 20.59 & 18.36   & 1.00 \\ % 8602
...
\begin{table}[H]
	\centering
	\caption{Solutions with $surplus=2.0$\/ with the direct and reverse least cost tours.}  \label{tab:100}
	\footnotesize
	\begin{tabular}{c|c|ccccc|c}
		\toprule
		$method$          & {\bf Scenarios} & {\bf 1} & {\bf 2} & {\bf 3} & {\bf 4} & {\bf 5} & \specialcell{{\bf Normalized}\\{\bf Speed-up}}   \\
		\toprule
		\multirow{2}{*}{\it Gurobi* } & $f$ &  &  &  &  &  &  \\ % 
		...
		\multirow{2}{*}{\it Gurobi* } & $f$ &  &  &  &  &    &  \\ % 
		&               {\bf run time (s)}  & 53    & 61    & 193   & 470   & 2241    &   \\ % 
		\midrule[.1pt]	
		\multirow{2}{*}{\it Shims}    & $f$ &  &  &  &  &    &   \\ % 
...
ESTA TABELA DEVERÁ REFEITA DEPOIS QUE AS ANTERIORES ESTIVEREM PRONTAS.
...
\bibitem{mipgap} What is the MIPGap? \url{https://support.gurobi.com/hc/en-us/articles/8265539575953-What-is-the-MIPGap-}, 2023.
