...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\For {each $method$} \Comment{$method$ is a MIP solver or a heuristic}
			...
			\State $f_{\pi_{TSP}} \gets SolveTour(\pi_{TSP}, L, M, C, N, method, time/(K+1))$ \label{main:f1}
			\State $answer1[scenario,surplus,method] \gets f_{\pi_{TSP}}$ \label{main:a1} \Comment{Results corresponding to $\pi_{TSP}$}
			\For {each $\pi \in S_K$} \label{main:loop1} \Comment{$\pi$ is a permutation of nodes (a tour)}
				\State $f_{\pi} \gets SolveTour(\pi, L, M, C, N, method, time/K!)$ \Comment{$f_{\pi}$\/ is a solution to the tour $\pi$} \label{main:f2}
...
$time$\/ is a runtime limit, which will be distributed among the node-by-node solutions (lines \ref{main:f1} and \ref{main:f2}). Each tour consists of $K+1$\/ trips, and the total number of tours is $K!$.
...
We solved all scenarios and instances with Gurobi in a relaxed linear model without a time limit. Although the solutions are not guaranteed to be feasible, it is good for the reader to have an idea of the possible upper bound for the metaheuristic solutions.

Note that, in Table \ref{tab:20} all $Gurobi\ f$\/ values are italicized to indicate that these values may not be optimal or feasible as the decision variables are not integer.
...
\midrule[.1pt]
\multirow{2}{*}{{\it Gurobi}}     & Upper bound for $f$      & {\it 7.91} & {\it 11.92} & {\it 13.12} & {\it 14.29} & {\it 13.34} & -  \\ %51,84
&{\bf run time (s)}                          & 14         & 19          & 73          & 188         & 1,097      & -  \\ 
\toprule
\multirow{2}{*}{\textbf{TS}}      & $f$      & 5.37       &             &             &             &            &   \\ % 5.49	 901
&{\bf run time (s)}                          & 659        &             &             &             &            &   \\ 
\midrule[.1pt]		
\multirow{2}{*}{\textbf{GRASP}}   & $f$      &            &             &             &             &            &   \\ 
&{\bf run time (s)}                          &            &             &             &             &            &   \\ 
\midrule[.1pt]
\multirow{2}{*}{\textbf{ACO}}     & $f$      & 5.25       & 5.75        & 5.06        & 4.84        &  4.20      &   \\ % 4.20	 1133
&{\bf run time (s)}                          & 313        & 326         & 303         & 256         &  1,133     &   \\ 
\midrule[.1pt]	
\multirow{2}{*}{\textbf{NMO}}     & $f$      &            &             &             &             &            &   \\
&{\bf run time (s)}                          &            &             &             &             &            &   \\ 
\midrule[.1pt]	
\multirow{2}{*}{\textbf{Shims}}   & $f$      & 7.91       & 11.55       & 12.62       &  13.82      &  14.85          &   \\ % 14.85	 563
&{\bf run time (s)}                          & 4          &  8          &  33         &  94         &  563          &   \\ 
...
\begin{table}[H]
	\centering
	\caption{Methods and scenarios with $surplus =1.5$}  \label{tab:50}
	\footnotesize
	\begin{tabular}{c|c|ccccc|c}
		\toprule
		{$method$}                        & {\bf Results}   & {\bf 1}     &  {\bf 2}    & {\bf 3}      & {\bf 4}      & {\bf 5}    & \specialcell{{\bf Normalized}\\{\bf Speed-up}} \\
...
\begin{table}[H]
	\centering
	\caption{Methods and scenarios with $surplus = 2.0$}  \label{tab:100}
	\footnotesize
	\begin{tabular}{c|c|ccccc|c}
		\toprule
		{$method$}                        & {\bf Results}    & {\bf 1}     &  {\bf 2}   & {\bf 3}    & {\bf 4}    & {\bf 5}      & \specialcell{{\bf Normalized}\\{\bf Speed-up}} \\
