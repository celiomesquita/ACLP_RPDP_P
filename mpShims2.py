...
def Solve(pallets, items, cfg, k, threshold, secBreak, mode, solTorque, solDict, itemsDict): # items include kept on board
    ...
    d_items = [[] for _ in dests] # items with destination "d"

    for it in items:
        for x, d in enumerate(dests):
            if it.To == d:
                d_items[x].append(it)
    ...
    if mode == "p":
        ...
        for i, p in enumerate(pallets):
            x = dests.index( p.Dest[k] )
            procs[i] = mp.Process( target=common.fillPallet, args=( pallets[i], d_items[x], k,\
        ...
        # for x, d in enumerate(dests):
        #     dests[x] = mp.Process( target=getBestShims, args=( pallets[i], items, k,\
    ...
    #         if X_ij == 0 and pallets[i].isFeasible(items[j], threshold, k, solTorque, solDict, lock, cfg, N, itemsDict):
