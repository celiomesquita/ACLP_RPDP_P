...
maxTorque = 75_000 * 1.170

class Knapsack:
    def __init__(self, dist_cg, knapsack_id, max_weight, max_volume ):
    ...
    def can_add_item(self, item):
        can_add = (self.current_weight + item.weight         <= self.max_weight) and\
              ...
              (abs(self.torque + self.dist_cg * item.weight) <= maxTorque)
        return can_add
    ...
    def add_item(self, item):
        if self.can_add_item(item):
            ...
            self.torque         += self.dist_cg * item.weight
            return True
        return False
