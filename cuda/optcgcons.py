...
import math
from mip import Model, xsum, minimize, BINARY, CBC, GRB, OptimizationStatus
...
if __name__ == "__main__":
    ...
    nix        = tour.index(nodeID) # node index in the tour
    unattended = tour[nix+1:]

    node = mcuda.Node(nodeID, 0.)
    ...
    cons.append( mcuda.Item(id+0, -1, 1897, 1387, 7.0,  0, 1) )
    cons.append( mcuda.Item(id+1, -1, 1962, 1221, 7.0,  0, 2) )
    cons.append( mcuda.Item(id+2, -1, 2613, 1486, 10.0, 0, 3) )
    cons.append( mcuda.Item(id+3, -1, 2666, 1650, 10.0, 0, 4) )
    cons.append( mcuda.Item(id+4, -1, 4153, 2672, 14.8, 0, 1) )
    cons.append( mcuda.Item(id+5, -1, 4053, 2085, 14.8, 0, 2) )
    cons.append( mcuda.Item(id+6, -1, 3562, 1967, 14.8, 0, 3) )
    cons.append( mcuda.Item(id+7, -1, 3926, 2405, 14.8, 0, 4) )
    cons.append( mcuda.Item(id+8, -1, 3593, 2606, 14.8, 0, 1) )
    cons.append( mcuda.Item(id+9, -1, 4145, 2440, 14.8, 0, 2) )
    cons.append( mcuda.Item(id+10, -1, 3912, 1990, 14.8, 0, 3) )
    cons.append( mcuda.Item(id+11, -1, 3774, 2136, 14.8, 0, 4) )
    cons.append( mcuda.Item(id+12, -1, 3745, 1975, 14.8, 0, 1) )
    cons.append( mcuda.Item(id+13, -1, 3670, 1740, 14.8, 0, 2) )
    cons.append( mcuda.Item(id+14, -1, 3410, 1937, 14.8, 0, 3) )
    cons.append( mcuda.Item(id+15, -1, 3542, 2381, 14.8, 0, 4) )
    cons.append( mcuda.Item(id+16, -1, 3306, 2248, 14.8, 0, 1) )
    cons.append( mcuda.Item(id+17, -1, 3385, 1985, 14.8, 0, 2) )
    ...
    for method in ["CBC","GRB"]:

        start_time = time()
        
        torque = OptCGCons(kept, pallets, cfg.maxTorque, method, nix)

        elapsed = time() - start_time
        elapsed = math.ceil(1000*elapsed)  # milliseconds    

        print(f"\nIt took {elapsed} ms with {method}: torque = {torque:.3f}\n")