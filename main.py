...
def solveTour(scenario, inst, pi, tour, method, pallets, cfg, secBreak, surplus):
    ...
    for k, node in enumerate(tour.nodes):  # solve each node sequentialy
        ...
        if method == "Shims_mp":
            shims_mp.Solve(pallets, items, cfg, k, 0.95, secBreak, "p", solTorque, solItems)

        if method == "Shims":            
            shims_mp.Solve(pallets, items, cfg, k, 0.95, secBreak, "s", solTorque, solItems)         

        if method == "ACO_mp":       
            aco_mp.Solve(pallets,   items, cfg, k, 0.0, secBreak, "p", solTorque, dictItems) 

        if method == "ACO":       
            aco_mp.Solve(pallets,   items, cfg, k, 0.0, secBreak, "s", solTorque, dictItems) 
